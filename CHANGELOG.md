...
- **Group passport on start**: optional `groupId` / `game` / `name` / `size` / `types` on `start` (and `setPassport`); echoed on retained state and gameplay JSONL (`session_header` + per-line join keys). Cleared on reset.
